...
const compressCarPhoto=file=>new Promise((resolve,reject)=>{let img=new Image(),url=URL.createObjectURL(file);img.onload=()=>{let max=1000,scale=Math.min(1,max/Math.max(img.width,img.height)),canvas=document.createElement('canvas');canvas.width=Math.round(img.width*scale);canvas.height=Math.round(img.height*scale);canvas.getContext('2d').drawImage(img,0,0,canvas.width,canvas.height);let data=canvas.toDataURL('image/jpeg',.7);URL.revokeObjectURL(url);resolve(data)};img.onerror=reject;img.src=url});
carImage.addEventListener('change',async()=>{let files=[...carImage.files].slice(0,8);if(files.some(f=>f.size>10_000_000))return toast('Каждое фото должно быть меньше 10 МБ');toast('Подготавливаем фотографии…');try{krugImagesData=await Promise.all(files.map(compressCarPhoto));krugImageData=krugImagesData[0]||'';carPreview.hidden=true;photoPreviews.innerHTML=krugImagesData.map((src,i)=>`<img src="${src}" alt="Фото ${i+1}">`).join('');toast(`Добавлено фото: ${krugImagesData.length} из 8`)}catch(e){toast('Не удалось обработать фотографию')}});
