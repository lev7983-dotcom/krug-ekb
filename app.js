...
function krugRenderPhotoPreviews(){let bytes=krugImagesData.reduce((sum,src)=>sum+Math.ceil(String(src).length*.75),0),size=bytes>=1_000_000?`${(bytes/1_000_000).toFixed(1)} МБ`:`${Math.max(1,Math.round(bytes/1000))} КБ`;photoPreviews.innerHTML=krugImagesData.map((src,i)=>`<div class="photo-preview-item"><img src="${safeImageSrc(src)}" alt="Фото ${i+1}"><button type="button" data-remove-photo="${i}" aria-label="Удалить фото ${i+1}">×</button>${i===0?'<small>Обложка</small>':''}</div>`).join('')+(krugImagesData.length?`<div class="photo-previews-size">${krugImagesData.length}/8<br>${size}</div>`:'')}
carImage.addEventListener('change',async()=>{let files=[...carImage.files].slice(0,8);if(files.some(f=>f.size>10_000_000))return toast('Каждое фото должно быть меньше 10 МБ');toast('Подготавливаем фотографии…');try{krugImagesData=await Promise.all(files.map(compressCarPhoto));krugImageData=krugImagesData[0]||'';carPreview.hidden=true;krugRenderPhotoPreviews();toast(`Добавлено фото: ${krugImagesData.length} из 8`)}catch(e){toast('Не удалось обработать фотографию')}});
...
const krugDraftNote=document.createElement('div');krugDraftNote.className='draft-note';krugDraftNote.innerHTML='<span>Черновик автоматически сохраняется</span><button type="button">Очистить</button>';document.querySelector('#create .page-head').after(krugDraftNote);
function krugDraftSnapshot(){return {name:carName.value,year:carYear.value,price:carPrice.value,km:carKm.value,description:carDescription.value,transmission:carTransmission.value,body_type:carBodyType.value,drive:carDrive.value,type:[...document.querySelectorAll('.type-card')].findIndex(x=>x.classList.contains('active')),saved_at:new Date().toISOString()}}
function saveKrugDraft(){if(krugEditingId)return;let draft=krugDraftSnapshot(),hasData=Object.entries(draft).some(([k,v])=>!['saved_at','type'].includes(k)&&String(v).trim());if(hasData){localStorage.setItem(KRUG_DRAFT_KEY,JSON.stringify(draft));krugDraftNote.classList.add('show')}else{localStorage.removeItem(KRUG_DRAFT_KEY);krugDraftNote.classList.remove('show')}}
function restoreKrugDraft(){try{let d=JSON.parse(localStorage.getItem(KRUG_DRAFT_KEY)||'null');if(!d)return;delete d.phone;delete d.vin;localStorage.setItem(KRUG_DRAFT_KEY,JSON.stringify(d));carName.value=d.name||'';carYear.value=d.year||'';carPrice.value=d.price||'';carKm.value=d.km||'';carDescription.value=d.description||'';carTransmission.value=d.transmission||'';carBodyType.value=d.body_type||'';carDrive.value=d.drive||'';let types=document.querySelectorAll('.type-card');types.forEach((x,i)=>x.classList.toggle('active',i===Number(d.type||0)));krugDraftNote.classList.add('show');toast('Черновик без телефона и VIN восстановлен')}catch(_){localStorage.removeItem(KRUG_DRAFT_KEY)}}
...
const makeKrugThumbnail=file=>new Promise((resolve,reject)=>{let img=new Image(),url=URL.createObjectURL(file);img.onload=()=>{let max=420,scale=Math.min(1,max/Math.max(img.width,img.height)),canvas=document.createElement('canvas');canvas.width=Math.round(img.width*scale);canvas.height=Math.round(img.height*scale);canvas.getContext('2d').drawImage(img,0,0,canvas.width,canvas.height);URL.revokeObjectURL(url);resolve(canvas.toDataURL('image/jpeg',.62))};img.onerror=reject;img.src=url});
carImage.addEventListener('change',async()=>{let first=carImage.files?.[0];if(!first){krugThumbnailData='';return}try{krugThumbnailData=await makeKrugThumbnail(first)}catch(_){krugThumbnailData=''}});
...
loadKrugCarPage=async function(reset=false){
  ...
  let requestId=++krugServerSearchSeq,offset=reset?0:krugCatalogOffset;krugMoreButton.classList.add('busy');if(reset)krugFilterResult.textContent='Ищем автомобили…';
  ...
  catch(error){if(requestId===krugServerSearchSeq)toast(error.message)}finally{if(requestId===krugServerSearchSeq)krugMoreButton.classList.remove('busy')}
};
...
let krugLegalInfo={};
async function loadKrugLegalInfo(){try{let r=await fetch('/api/legal',{headers:{'Accept':'application/json'}});krugLegalInfo=await r.json();let name=krugLegalInfo.operator_name||'реквизиты ещё не заполнены',email=krugLegalInfo.operator_email||'контакт ещё не указан',address=krugLegalInfo.operator_address||'адрес ещё не указан';privacyOperatorText.textContent=krugLegalInfo.ready?`Оператор: ${name}. Адрес: ${address}. Контакт: ${email}. Первичное хранение данных в РФ подтверждено.`:'Сбор персональных данных отключён, пока владелец не укажет реквизиты и не подтвердит хранение базы в РФ.';return krugLegalInfo}catch(_){privacyOperatorText.textContent='Не удалось проверить юридические настройки сервиса.';return {ready:false}}}
...
photoPreviews.addEventListener('click',async event=>{let button=event.target.closest('[data-remove-photo]');if(!button)return;let index=Number(button.dataset.removePhoto);if(!Number.isInteger(index)||index<0||index>=krugImagesData.length)return;krugImagesData.splice(index,1);krugImageData=krugImagesData[0]||'';krugThumbnailData=await krugThumbnailFromData(krugImageData);krugRenderPhotoPreviews();carImage.value='';toast('Фотография удалена')});
