...
async function showMyCars(){
  try{let list=await krugJson('/api/my-cars');go('catalog');document.querySelector('#catalog .page-head h1').textContent='Мои объявления';document.getElementById('catalogCards').innerHTML=list.map(c=>`${card(c)}${c.status==='sold'?'<div class="sold-label">✓ Автомобиль продан</div>':`<div class="listing-insights"><span>👁 Просмотры: ${Number(c.views)||0}</span><span>♥ В избранном: ${Number(c.favourites_count)||0}</span></div>`}${c.status==='review'?'<div class="panel" style="margin-top:8px;background:#fff4d8"><b>На проверке</b><div class="meta">Объявление временно скрыто после жалоб.</div></div>':''}<div class="manage-actions">${c.status==='sold'?`<button class="btn back" onclick="manageCar(${c.id},'activate',this)">Вернуть в продажу</button>`:`<button class="btn lime" onclick="editCar(${c.id})">Изменить</button>${c.status==='review'?'':`<button class="btn back" onclick="manageCar(${c.id},'${c.status==='active'?'archive':'activate'}',this)">${c.status==='active'?'Снять':'Опубликовать'}</button><button class="btn sold-button" onclick="markCarSold(${c.id})">Продано</button>`}`}<button class="btn" style="background:#ffe3dc" onclick="deleteCar(${c.id})">Удалить</button></div>`).join('')||'<div class="panel"><h3>Объявлений пока нет</h3></div>'}catch(error){toast(error.message)}}
...
let krugModerationReports=[];
async function showModeration(){
  try{let reports=krugModerationReports=await krugJson('/api/admin/reports');go('catalog');document.querySelector('#catalog .page-head h1').textContent='Модерация';document.getElementById('catalogCards').innerHTML=reports.map(r=>`<div class="panel moderation-card"><div class="eyebrow"><span class="dot"></span>жалоба №${Number(r.id)}</div><div class="moderation-car"><img src="${safeImageSrc(r.car_image)}" alt="${safeText(r.car_name)}"><div><h3>${safeText(r.car_name)}</h3><div class="meta">${Number(r.car_year)||'Год не указан'} · ${safeText(r.car_km||'Пробег не указан')} · ${rub(Number(r.car_price)||0)}</div></div></div><span class="moderation-reason">${safeText(krugReportReasons[r.reason]||r.reason)}</span>${r.details?`<p>${safeText(r.details)}</p>`:''}<div class="meta">Отправитель: ${safeText(r.reporter_name||'Пользователь')}</div><button class="btn back moderation-open" onclick="openModerationCar(${Number(r.car_id)})">Посмотреть автомобиль</button><div class="manage-actions"><button class="btn lime" onclick="moderateReport(${Number(r.id)},'approve')">Вернуть объявление</button><button class="btn sold-button" onclick="moderateReport(${Number(r.id)},'block')">Заблокировать</button></div></div>`).join('')||'<div class="panel"><h3>Новых жалоб нет</h3></div>'}catch(error){toast(error.message)}
}
async function openModerationCar(id){let report=krugModerationReports.find(item=>Number(item.car_id)===Number(id));if(!report)return toast('Жалоба уже обновлена');await openCarV3(Number(id),report.car_name,Number(report.car_price)||0,'50% 50%',safeImageSrc(report.car_image))}
...
document.addEventListener('error',event=>{let image=event.target;if(image?.tagName==='IMG'&&!image.dataset.fallback){image.dataset.fallback='1';image.src='/krug-hero.webp'}},true);
...
loadKrugCarPage=async function(reset=false){
  ...
  let requestId=++krugServerSearchSeq,offset=reset?0:krugCatalogOffset;krugMoreButton.classList.add('busy');if(reset){krugFilterResult.textContent='Ищем автомобили…';if(!cars.length)['homeCards','catalogCards','urgentCards'].forEach(id=>document.getElementById(id).innerHTML='<div class="catalog-skeleton" aria-label="Загружаем автомобили"><i></i><i></i><i></i></div>')}
  try{let d=await krugJson(`/api/cars?${krugServerCatalogueQuery(offset)}`);if(requestId!==krugServerSearchSeq)return;let known=new Set((reset?[]:cars).map(car=>Number(car.id)));cars=reset?d.items:[...cars,...d.items.filter(car=>!known.has(Number(car.id)))];krugCatalogOffset=offset+d.items.length;krugCatalogHasMore=!!d.has_more;krugCatalogTotal=Number(d.total)||0;render(cars.slice(0,3),'homeCards');render(cars,'catalogCards');render(cars.filter(car=>car.urgent),'urgentCards');if(!cars.length)document.getElementById('catalogCards').innerHTML='<div class="panel search-empty"><h3>Автомобили не найдены</h3><p class="meta">Попробуйте изменить запрос или сбросить фильтры.</p><button class="btn back" type="button" onclick="document.querySelector(\'.filter-reset\').click()">Сбросить фильтры</button></div>';paintKrugMore()}
  catch(error){if(requestId===krugServerSearchSeq){krugFilterResult.textContent='Не удалось обновить каталог';if(!cars.length){let failure='<div class="panel catalog-error"><i>↻</i><h3>Не удалось загрузить автомобили</h3><p class="meta">Проверьте интернет. Если бесплатный сервер просыпается, повторите через несколько секунд.</p><button class="btn lime" type="button" onclick="loadKrugCarPage(true)">Попробовать снова</button></div>';['homeCards','catalogCards','urgentCards'].forEach(id=>document.getElementById(id).innerHTML=failure)}toast(error.message)}}finally{if(requestId===krugServerSearchSeq)krugMoreButton.classList.remove('busy')}
};
...
loadKrugCarPage=async function(reset=false){
  ...
  const run=(async()=>{
    ...
    if(reset&&krugFilterResult.textContent==='Не удалось обновить каталог'){
      ...
      if(cached?.items.length){cars=cached.items;krugCatalogOffset=cars.length;krugCatalogHasMore=false;krugCatalogTotal=Number(cached.total)||cars.length;render(cars.slice(0,3),'homeCards');render(cars,'catalogCards');render(cars.filter(car=>car.urgent),'urgentCards');paintKrugMore();krugFilterResult.textContent='Показан сохранённый каталог · обновим при подключении'}
    }
  })();
...
function finishKrugPersonalLoading(retryCall){if(!document.querySelector('#catalogCards .personal-skeleton'))return;document.querySelector('#catalog .page-head p').textContent='Личный раздел КРУГ';document.getElementById('catalogCards').innerHTML=`<div class="panel search-empty personal-load-error"><h3>Не удалось загрузить данные</h3><p class="meta">Проверьте интернет или повторите через несколько секунд.</p><button type="button" class="btn back" onclick="${retryCall}()">Повторить</button></div>`}
const krugShowMyCarsWhenReady=showMyCars;
showMyCars=async function(){showKrugPersonalLoading('Мои объявления');if(!await krugPersonalReady())return showKrugPersonalUnavailable('Мои объявления');let result=await krugShowMyCarsWhenReady();finishKrugPersonalLoading('showMyCars');return result};
const krugShowFavouritesWhenReady=showFavourites;
showFavourites=async function(){showKrugPersonalLoading('Избранное');if(!await krugPersonalReady())return showKrugPersonalUnavailable('Избранное');let result=await krugShowFavouritesWhenReady();finishKrugPersonalLoading('showFavourites');return result};
const krugShowRecentWhenReady=showRecentlyViewed;
showRecentlyViewed=async function(){showKrugPersonalLoading('Недавно просмотренные');if(!await krugPersonalReady())return showKrugPersonalUnavailable('Недавно просмотренные');let result=await krugShowRecentWhenReady();finishKrugPersonalLoading('showRecentlyViewed');return result};
const krugShowExchangesWhenReady=showExchanges;
showExchanges=async function(){showKrugPersonalLoading('Предложения обмена');if(!await krugPersonalReady())return showKrugExchangeSetup();let result=await krugShowExchangesWhenReady();finishKrugPersonalLoading('showExchanges');return result};
...
async function showPartnerSources(){
  try{
    ...
    document.getElementById('catalogCards').innerHTML=`<div class="panel"><div class="eyebrow"><span class="dot"></span>только с разрешения владельца</div><h3>Подключить источник</h3><p class="meta">Telegram: добавьте КРУГ администратором в группу или канал. В группе отправьте /krug_source; канал можно указать по ID ниже. VK подключается только через официальный Callback API — без парсинга страниц.</p><div class="team-form"><select id="partnerPlatform" onchange="paintPartnerFields()"><option value="telegram">Telegram-группа или канал</option><option value="vk">Сообщество VK</option></select><input id="partnerRef" inputmode="text" placeholder="ID группы, канала или сообщества"><input id="partnerTitle" placeholder="Понятное название"><div id="partnerVkFields" hidden><input id="partnerSecret" type="password" autocomplete="off" placeholder="Секретный ключ Callback API"><input id="partnerConfirmation" autocomplete="off" placeholder="Строка подтверждения VK"></div><button class="btn lime" onclick="addPartnerSource()">Сохранить источник</button></div></div>`+sources.map(s=>`<div class="panel"><div class="eyebrow"><span class="dot"></span>${s.platform==='telegram'?'Telegram':'VK'}</div><h3>${safeText(s.title||'Источник')}</h3><div class="meta">ID ${safeText(s.source_ref)} · ${s.status==='active'?'активен':'отключён'} · ${Number(s.configured)?'настроен':'требует настройки'}</div><div class="meta">Получено: ${Number(s.draft_count)||0} · ждут проверки: ${Number(s.pending_count)||0}${s.last_import_at?` · последний ${new Date(s.last_import_at).toLocaleDateString('ru-RU')}`:' · публикаций ещё не было'}</div>${s.last_event_at?`<div class="meta">Связь: ${s.last_error?`ошибка ${safeText(s.last_error)}`:'работает'} · ${new Date(s.last_event_at).toLocaleString('ru-RU')}</div>`:''}<div class="manage-actions">${s.status==='active'?`<button class="btn back" onclick="disablePartnerSource(${Number(s.id)})">Отключить</button>`:`<button class="btn lime" onclick="enablePartnerSource(${Number(s.id)})">Включить снова</button><button class="btn back" onclick="removePartnerSource(${Number(s.id)})">Удалить навсегда</button>`}</div></div>`).join('');paintPartnerFields();
  }catch(error){toast(error.message)}
}
function paintPartnerFields(){const vk=document.getElementById('partnerPlatform')?.value==='vk';if(document.getElementById('partnerVkFields'))document.getElementById('partnerVkFields').hidden=!vk}
async function addPartnerSource(){try{const platform=document.getElementById('partnerPlatform').value,source_ref=document.getElementById('partnerRef').value.trim(),title=document.getElementById('partnerTitle').value.trim(),callback_secret=document.getElementById('partnerSecret')?.value||'',confirmation_code=document.getElementById('partnerConfirmation')?.value||'';await krugJson('/api/admin/partner-sources',{method:'POST',body:JSON.stringify({platform,source_ref,title,callback_secret,confirmation_code})});toast('Источник сохранён');await showPartnerSources()}catch(error){toast(error.message)}}
...
async function enablePartnerSource(id){try{await krugJson(`/api/admin/partner-sources/${Number(id)}/enable`,{method:'POST',body:'{}'});toast('Источник снова включён');await showPartnerSources()}catch(error){toast(error.message)}}
function removePartnerSource(id){askKrugConfirm({title:'Удалить источник навсегда?',text:'Настройки подключения и сохранённые хеши секретов будут удалены. Уже созданные черновики останутся у их владельца.',action:'Удалить навсегда',danger:true,onConfirm:async()=>{await krugJson(`/api/admin/partner-sources/${Number(id)}/remove`,{method:'DELETE'});toast('Источник удалён');await showPartnerSources()}})}
...
/* KRUG source block 66 */
// v122: imported drafts stay reachable even after a Telegram notification is closed.
const krugImportsButton=document.createElement('button');krugImportsButton.className='menu-item admin-menu';krugImportsButton.innerHTML='Черновики из источников <span>0</span>';document.querySelector('#profile .menu-list').append(krugImportsButton);
async function showImportDrafts(){try{const drafts=await krugJson('/api/imports');go('catalog');document.querySelector('#catalog .page-head h1').textContent='Черновики';const labels={name:'модель',year:'год',price:'цена',km:'пробег',photo:'фото'},tools=drafts.length?`<div class="panel"><div class="meta">В очереди: ${drafts.length}</div><div class="manage-actions"><button class="btn back" onclick="clearImportDrafts()">Очистить очередь</button></div></div>`:'';document.getElementById('catalogCards').innerHTML=tools+drafts.map(d=>`<div class="panel"><div class="eyebrow"><span class="dot"></span>${safeText(d.source_title||(d.source_type.includes('vk')?'VK':'Telegram'))} · распознано ${Number(d.quality)||0}%</div><h3>${safeText(d.name||'Черновик объявления')}</h3><div class="meta">${Number(d.year)||'Год не определён'} · ${Number(d.price)?rub(Number(d.price)):'Цена не определена'}</div><div class="meta">Получен ${new Date(d.created_at).toLocaleDateString('ru-RU')} · хранится 7 дней</div>${d.missing?.length?`<div class="meta">Нужно добавить: ${d.missing.map(x=>labels[x]||x).join(', ')}</div>`:'<div class="meta">Основные данные распознаны</div>'}<div class="manage-actions"><button class="btn lime" onclick="openImportDraft(${Number(d.id)})">Проверить</button>${d.source_url?`<button class="btn back" data-import-source="${safeText(d.source_url)}" onclick="openImportSource(this.dataset.importSource)">Оригинал</button>`:''}<button class="btn back" onclick="removeImportDraft(${Number(d.id)})">Удалить</button></div></div>`).join('')||'<div class="panel empty-state"><h3>Черновиков пока нет</h3><p class="meta">Перешлите объявление боту или подключите партнёрскую группу.</p></div>'}catch(error){toast(error.message)}}
function openImportSource(value){try{const url=new URL(String(value||'')),host=url.hostname.toLowerCase();if(url.protocol!=='https:'||!['vk.com','www.vk.com','t.me'].includes(host))throw Error('blocked');if(host==='t.me'&&window.Telegram?.WebApp?.openTelegramLink)Telegram.WebApp.openTelegramLink(url.href);else if(window.Telegram?.WebApp?.openLink)Telegram.WebApp.openLink(url.href);else window.open(url.href,'_blank','noopener,noreferrer')}catch(_){toast('Небезопасная ссылка заблокирована')}}
...
function clearImportDrafts(){askKrugConfirm({title:'Очистить очередь?',text:'Все непроверенные импортированные черновики будут удалены без возможности восстановления.',action:'Очистить',danger:true,onConfirm:async()=>{const result=await krugJson('/api/imports',{method:'DELETE'});krugActiveImportId=0;krugImportLoaded=0;toast(`Удалено черновиков: ${Number(result.deleted)||0}`);await showImportDrafts()}})}
