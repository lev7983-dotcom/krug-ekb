...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path=="/api/health": return self.send_json({"ok":True,"service":"krug","version":73,"release":APP_RELEASE,"production":PRODUCTION,"personal_actions":bool(LEGAL_READY or OPEN_BETA),"testing_mode":OPEN_BETA,"closed_beta":bool(TESTER_IDS and not OPEN_BETA),"telegram":dict(TELEGRAM_STATUS)})
        if path=="/api/legal":
