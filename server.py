...
def parse_imported_listing(text):
    ...
    colors=r"ч[её]рный|белый|серый|серебристый|синий|голубой|красный|зел[её]ный|бежевый|коричневый|ж[её]лтый|оранжевый|фиолетовый"
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path=="/api/health":
            ...
            return self.send_json({"ok":database_ok,"service":"krug","version":91,"release":APP_RELEASE,"commit":DEPLOY_COMMIT,"uptime_seconds":max(0,int(time.time()-PROCESS_STARTED_AT)),"production":PRODUCTION,"database_ok":database_ok,"personal_actions":bool(LEGAL_READY or OPEN_BETA),"testing_mode":OPEN_BETA,"closed_beta":bool(TESTER_IDS and not OPEN_BETA),"telegram":dict(TELEGRAM_STATUS)},200 if database_ok else 503)
        if path=="/api/legal":
